import numpy as np
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import librosa
import librosa.display
import os
from PIL import Image
from scipy.io import wavfile

# define conversion function
def convert_wav_to_spectrogram(wav_path, output_path, sr=22050, n_mels=128, n_fft=2048, hop_length=512, size=(128, 128)):
    try:
        samplerate,audio_data = wavfile.read(wav_path)
        # if audio data is integer type, normalize to float
        if np.issubdtype(audio_data.dtype, np.integer):
            audio_data = audio_data.astype(np.float32) / np.max(np.abs(audio_data))
        # calculate Mel spectrogram
        S = librosa.feature.melspectrogram(y=audio_data, sr=sr, n_mels=n_mels, n_fft=n_fft, hop_length=hop_length)
        S_dB = librosa.power_to_db(S, ref=np.max)
        # display Mel spectrogram
        fig, ax = plt.subplots(figsize=(size[0] / 100, size[1] / 100), dpi=100)
        img = librosa.display.specshow(S_dB, x_axis='time', y_axis='mel', sr=sr, fmax=10000, ax=ax)
        # remove axes
        ax.axis('off')
        # save image as PNG file
        plt.savefig('temp.png', bbox_inches='tight', pad_inches=0, dpi=100)
        plt.close()
        # read image and resize
        with Image.open('temp.png') as img:
            img = img.resize(size, Image.LANCZOS)
            img.save(output_path)

        # delete temporary file
        os.remove('temp.png')
    except Exception as e:
        print(f"Error processing {wav_path}: {e}")


# data paths
#base_folder = 'C:\\Users\\User\\Desktop\\Dataset\\bird song data set\\selected_wavfiles_label'
base_folder = './data/selected_wavfiles_label'
output_folder_name = 'BSC5_Mel_Spectrograms_128'
parent_dir = os.path.dirname(base_folder)
output_folder = os.path.join(parent_dir, output_folder_name)
# create output directory
os.makedirs(output_folder, exist_ok=True)
print(f"Output will be saved in: {output_folder}")

# process each file
for file_name in os.listdir(base_folder):
    if file_name.endswith('.wav'):
        wav_path = os.path.join(base_folder, file_name)
        
        output_path = os.path.join(output_folder, f"{os.path.splitext(file_name)[0]}.png")
        convert_wav_to_spectrogram(wav_path, output_path)

print("Conversion completed.")