...
# 資料夾路徑
folder_path = 'C:\\Users\\User\\Desktop\\Dataset\\ESC-50-master\\audio'
output_folder = os.path.join(folder_path, 'ESC50_Adaf_Spectrograms_128(Amplitude)')
# 如果儲存圖片的資料夾不存在，則創建
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".wav"):
            ...
            try:
                ...
                # frequency_energy = np.sum(np.abs(spectrogram)**2, axis=1) # Power
            ...
            try:
                ...
                for i in range(num_sections):
                    ...
                    if len(freq_indices) > 0:
                        ...
                        # adaptive_spectrogram[i] = np.sum(np.abs(spectrogram[freq_indices, :])**2, axis=0)     # Power
                ...
                # S_db_1 = librosa.amplitude_to_db(adaptive_spectrogram, ref=np.max)                            # dB
